...
export default function MasterlistProfileModal({ application, isOpen, onClose, onSave, onRefresh, onMoveToPending, onResetPassword }: MasterlistProfileModalProps) {
  ...
  const isTransferred = ((application as any).transferred_to || '').toString().trim() !== '';
  const canEdit = [1, 2].includes(userRole) && !isTransferred;
  const canMoveToPending = [1, 2, 3].includes(userRole) && !isTransferred;
          ...
          <div className="space-y-8 pb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {(() => {
                ...
                const proofOfTransfer = (application as any).proof_of_transfer;
                
                return (
                  <>
                    {proofOfTransfer && (
                      <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl flex flex-col gap-3 relative group/file">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center shadow-sm">
                            <FileText className="w-4 h-4 text-rose-500" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-[10px] font-black text-rose-900 truncate uppercase tracking-tight">Proof of Transfer</p>
                            <p className="text-[8px] font-bold text-rose-400 uppercase tracking-widest">Attachment</p>
                          </div>
                        </div>
                        <div className="flex gap-2">
                          <button 
                            type="button"
                            onClick={() => {
                              const isPdf = proofOfTransfer.startsWith('data:application/pdf') || proofOfTransfer.toLowerCase().includes('.pdf');
                              setViewingFile({
                                url: proofOfTransfer,
                                filename: 'Proof_of_Transfer',
                                type: isPdf ? 'application/pdf' : 'image/png'
                              });
                            }}
                            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 bg-rose-600 text-white rounded-lg font-black text-[8px] uppercase tracking-widest hover:bg-rose-700 transition-all cursor-pointer"
                          >
                            <Eye className="w-2.5 h-2.5" />
                            VIEW
                          </button>
                          <a 
                            href={proofOfTransfer}
                            download="proof_of_transfer"
                            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white text-rose-600 border border-rose-200 rounded-lg font-black text-[8px] uppercase tracking-widest hover:bg-rose-50 transition-all cursor-pointer text-center"
                          >
                    ...
                    {attachments.length === 0 && newFiles.length === 0 && !proofOfTransfer && (
