  ...
  */
  ...
  app.put("/api/masterlist/:id", (req, res) => {
    ...
    const index = masterlist.findIndex(m => 
      m.id === masterId || 
      m.citizen_id === masterId || 
      m.id?.toString() === id.toString() || 
      m.citizen_id?.toString() === id.toString()
    );
    if (index !== -1) {
  ...
  // --- EXPANDED CENTENARIANS ACT BENEFIT PROGRAM ENDPOINTS ---

  const centenarianApplications: any[] = [];

  // GET all centenarian applications
  app.get("/api/expanded-centenarians", (req, res) => {
    res.json(centenarianApplications);
  });

  // GET specific centenarian application by id
  ...
  // POST create centenarian application
  ...
  // PUT update centenarian application
  ...
  // DELETE centenarian application
